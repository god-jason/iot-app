<template>
	<view class="page">
		<uni-card title="转移组织">
			<uni-forms :modelValue="formData" :label-width="100">
				<uni-forms-item label="手机号" required name="cellphone">
					<uni-easyinput v-model="formData.cellphone" placeholder="请输入目标用户手机号" />
				</uni-forms-item>
			</uni-forms>
		...
		<uni-card v-if="targetUser" title="用户信息">
			...
			<view class="button-group">
				<button type="warn" :loading="transferring" @click="confirmTransfer">确认转移</button>
			</view>
		</uni-card>
	</view>
</template>
			...
			async doTransfer() {
				...
				try {
					...
					if (res && (res.data === 0 || res.data > 0 || res.code === 0)) {
						...
						uni.showToast({
							title: res.message || '转移失败',
							icon: 'error'
						});
					}
				} catch (error) {
					console.error('转移组织失败:', error);
					uni.showToast({
						title: '转移失败',
						icon: 'error'
					});
